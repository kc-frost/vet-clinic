...
import { getInventory, createInventoryItem, deleteInventoryItem, updateInventoryItem } from "../../api/inventory";
...
export default function Inventory() {
  ...
  async function handleToggleInUse(item: InventoryItem) {
    setPageError("");
    if (item.inventoryType !== "equipment") return;

    setLoading(true);
    try {
      await updateInventoryItem(item.itemID, { inUse: !item.inUse });
      await refreshAll();
    } catch (err) {
      setPageError(errMsg(err));
      setLoading(false);
        ...
        <section className="card">
          ...
          <div className="form">
            ...
            <select
              className="input"
              value={iType}
              onChange={(e) => handleInventoryTypeChange(e.target.value as InventoryType)}
              disabled={loading}
            >
            ...
            {iType === "equipment" ? (
              <label className="checkboxRow">
                <input
                  type="checkbox"
                  checked={iInUse}
                  onChange={(e) => setIInUse(e.target.checked)}
                  disabled={loading}
                />
                In use
              </label>
          ...
          <div className="list">
            {itemsSorted.map((it) => (
              <div key={it.itemID} className="row">
                ...
                <div className="row-actions">
                  {it.inventoryType === "equipment" ? (
                    <button className="btn" type="button" onClick={() => handleToggleInUse(it)} disabled={loading}>
                      Toggle In Use
                    </button>
                  ) : null}
                  <button
